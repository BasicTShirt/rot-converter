...
def main():
    parser = argparse.ArgumentParser(
        ...
        description=(
            ...
            "Encrypt, decrypt, bruteforce or auto-detect ROT "
            "ciphers for 20+ languages.\n"
    ...
    smart.add_argument(
        ...
        required=True,
        help=(
            "Language code (frequency analysis"
            "only works for built-in languages)"
        )
    )
    smart.add_argument(
    ...
    if args.command is None:
        print("rot-converter Copyright (C) 2026 BasicSweater")
        print(
            "This program comes with ABSOLUTELY NO WARRANTY; "
            "for details type `show w`."
        )
        print("This is free software, and you are welcome to redistribute it")
        print("under certain conditions; for details type `show c`.\n")
        parser.print_help()
        sys.exit(0)
    ...
    try:
        if args.command == "encrypt":
            ...
            print("-" * 60)
            for rank, (shift, text, score) in enumerate(
                guesses[:top_n], start=1
            ):
