...
class ROTConvertor:
    ...
    def custom_alphabet_handler(
        ...
        self.general_rot_alphabet = (
            self.alphabets[rot_language_or_custom_alphabet] 
            if self.alphabet_state
    ...
    def _combine_error_handler(
        ...
        if (
            ...
            errors.append(
                "ERROR! You did not enter a language or a "
                "custom alphabet"
            )
            local_state_1 = False
        if (
            ...
            errors.append(
                 "ERROR! Type of the language or the custom "
                 "alphabet is not a string"
            )
            local_state_1 = False
        ...
        if (
            ...
            upper_alphabet = rot_language_or_custom_alphabet.upper()
            if (
                len(set(upper_alphabet))
                != len(rot_language_or_custom_alphabet)
            ):
                errors.append(
                    "ERROR! Custom alphabet contains duplicate characters"
                )
    ...
    def universal_rot_handler(
        ...
        for symbol in local_rot_string:
            if symbol.upper() in local_rot_alphabet:
                ...
                index = (
                    self.control_dict[rot_language_or_custom_alphabet][
                         symbol.upper()
                    ]
                    if self.alphabet_state
    ...
    def b_rot(
        ...
        lang_str = (
                   rot_language_or_custom_alphabet 
                   if rot_language_or_custom_alphabet is not None 
                   else ""
        )
    ...
    def _smart_mode_score(
        ...
        return float(dot_product / (norm_text ** 0.5 * norm_ref ** 0.5))

def m_rot(s, n, lang):
    converter = ROTConvertor()
    return converter.m_rot(s, n, lang)

def b_rot(s, lang):
    converter = ROTConvertor()
    return converter.b_rot(s, lang)

def smart_rot(s, lang):
    converter = ROTConvertor()
    return converter.smart_rot(s, lang)
